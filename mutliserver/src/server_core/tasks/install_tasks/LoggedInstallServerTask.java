...
public final class LoggedInstallServerTask extends InstallationTask {
    private final Logger LOG = LoggerFactory.getLogger(InstallationTask.class);

    public LoggedInstallServerTask(InputStream dataSource, OutputStream channel, ServerConfigurators config) {
        super(dataSource, channel, config);
    }

    @Override
    public BaseServerParameters promptBaseServerParameters(InputStream in, OutputStream out) {
        LOG.info("Getting base server parameters like IP and port...");
        LOG.info("Prompting server address from source...");
        String serverAddress = promptServerAddress(in, out);
        LOG.info("Prompting server port from source...");
        int serverPort = promptServerPort(in, out);
        LOG.info("Finishing getting base parameters...");
    ...
    public String promptServerAddress(InputStream in, OutputStream out) {
        try {
            ...
            LOG.error("Cannot get server address from current streams\n");
            return BaseServerParameters.DEFAULT_IP_ADDRESS;
        }
    }
    ...
    public int promptServerPort(InputStream in, OutputStream out) {
        try {
            ...
            LOG.error("Cannot get server port from current streams\n");
            return BaseServerParameters.DEFAULT_PORT;
        }
    }
}
