...
public class RoofBuilder extends RoomBuilder {
    ...
    public void buildFacade(double x, double y, double width, double height, Paint fill) {
        product.installFacade(new Facade(x + width / 2, y + height / 2, width / 2, height / 2, fill));
    }
    ...
    public void buildHBeams(double x, double y, double width, double height, Paint fill) {
        product.installHBeam(BOTTOM_BEAM, new Beam(x + width / 2 - width / 10, y + height - 1, width * 1.2 / 2, height / 20, fill));
        product.installHBeam(TOP_BEAM, new Beam(x + width / 2 - width / 10, y + height / 2 - 1, width * 1.2 / 2, height / 20, fill));
    }

    @Override
    public void buildVBeams(double x, double y, double width, double height, Paint fill) {
        product.installVBeam(LEFT_BEAM, new Beam(x + width / 2, y + height / 2, width / 20, height / 2, fill));
        product.installVBeam(RIGHT_BEAM, new Beam(x + width, y + height / 2, width / 20, height / 2, fill));
    }

    @Override
    public void buildFlags(double x, double y, double width, double height, Paint fill) {
        product.installFlag(0, new Flag(x + 0.75 * width, y + height / 2, width / 2, height / 2, fill));
    }
}
